...
# qwencraft-net binary via include_dir!, so after running this script the
# server must be rebuilt (cargo build -p qwencraft-net).
#
...
wasm-bindgen --target web --out-dir "$DASH/dist" \
  "$TARGET/qwencraft_dashboard.wasm"
...
echo "  cargo build -p qwencraft-net"
