...
// `TERRAIN_POOL_VERTS`/`TERRAIN_POOL_IDX` (qwencraft_world).
//
...
// view-radius changes).
//
//   cargo run --release -p qwencraft-server --example pool_measure
use qwencraft_server::{Server, World, VIEW_RADIUS};
use qwencraft_world::{ChunkPos, TERRAIN_POOL_IDX, TERRAIN_POOL_VERTS, WORLD_HEIGHT};
...
fn main() {
    ...
    for seed in seeds {
        ...
        for &(ox, oz) in &positions {
            let pc = ChunkPos::of(qwencraft_world::BlockPos::new(ox, 0, oz));
    ...
    assert!(
        ...
        "worst view uses {v_pct:.0}% of VERT_CAP (> 80%): raise TERRAIN_POOL_VERTS/IDX_CAP in qwencraft-world and re-verify the walk test"
    );
    assert!(
        worst_i as f64 <= TERRAIN_POOL_IDX as f64 * 0.8,
        "worst view uses {i_pct:.0}% of IDX_CAP (> 80%): raise TERRAIN_POOL_VERTS/IDX_CAP in qwencraft-world and re-verify the walk test"
    );
