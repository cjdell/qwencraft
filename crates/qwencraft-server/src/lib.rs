...
impl Server {
    ...
    fn spawnable(world: &World, x: i32, z: i32) -> bool {
        ...
        h >= qwencraft_world::SEA_LEVEL
            && h < qwencraft_world::SNOW_LEVEL
    ...
    fn apply_world_edit(&mut self, id: u32, action: Action) {
        ...
        let d = qwencraft_world::camera::look_direction(yaw, pitch);
...
impl Streamer {
    ...
    fn region_ready(world: &World, c: ChunkPos) -> bool {
        for k in Self::context_chunks(c) {
            ...
            if k.y * qwencraft_world::CHUNK < 0 || k.y * qwencraft_world::CHUNK >= WORLD_HEIGHT {
...
mod tests {
    use super::*;
    use qwencraft_world::{Block, BlockPos, REGION_BLOCKS, TERRAIN_POOL_IDX, TERRAIN_POOL_VERTS, WORLD_HEIGHT};
    ...
    fn worst_view_fits_terrain_pool_with_headroom() {
            ...
            for dx in -VIEW_RADIUS..=VIEW_RADIUS {
                for dz in -VIEW_RADIUS..=VIEW_RADIUS {
                    ...
                    for cy in 0..(WORLD_HEIGHT / qwencraft_world::CHUNK) {
                        ...
                        let mesh = qwencraft_world::mesh::build_chunk_mesh(
        ...
        assert!(
            ...
            "worst pinned view ({}) uses {v_pct:.0}% of TERRAIN_POOL_VERTS (> 80%): raise the caps in qwencraft-world or re-run pool_measure",
            worst.2
        );
        assert!(
            worst.1 as f64 <= TERRAIN_POOL_IDX as f64 * 0.8,
            "worst pinned view ({}) uses {i_pct:.0}% of TERRAIN_POOL_IDX (> 80%): raise the caps in qwencraft-world or re-run pool_measure",
            worst.2
        );
    }
    ...
    fn evicted_chunks_are_re_streamed_when_visible_again() {
        ...
        // visible.
        let own = qwencraft_world::ChunkPos::of(qwencraft_world::BlockPos::new(
            p.x as i32,
            p.y as i32,
            p.z as i32,
        ));
        ...
        let far = qwencraft_world::ChunkPos::new(
            own.x + VIEW_RADIUS + 5,
            0,
            own.z + VIEW_RADIUS + 5,
        );
    ...
    fn fog_chunk_distance_invariant() {
        ...
        // blocks away.
        use qwencraft_world::camera::FOG_END;
    ...
    fn swimming_sinks_slowly_and_rises_with_space() {
        ...
        let (lx, lz) = (0..4096)
            .find_map(|i| {
                ...
                (s.world.height_at(x, z) < qwencraft_world::SEA_LEVEL - 2).then_some((x, z))
            })
        ...
        let sea = qwencraft_world::SEA_LEVEL;
    ...
    fn edits_go_through_delta_layer() {
        ...
        let generated_before = s.world.contains(&qwencraft_world::ChunkPos::of(target));
        if generated_before {
    ...
    fn break_and_place_update_world() {
        ...
        let d = qwencraft_world::camera::look_direction(yaw, TEST_PITCH);
        let dir = Vec3::new(d[0], d[1], d[2]);
        let (target, _) = s
            .world
    ...
    fn break_uses_click_time_aim_not_current_aim() {
        ...
        let d0 = qwencraft_world::camera::look_direction(yaw0, pitch0);
        ...
        for k in 1..=8 {
            ...
            let d1 = qwencraft_world::camera::look_direction(yaw1, pitch0);
